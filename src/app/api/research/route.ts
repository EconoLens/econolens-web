...
export async function POST(req: NextRequest) {
  ...
  if (query.length > 600) return NextResponse.json({ error: "Query too long" }, { status: 400 });
  ...
  try {
    ...
    if (cached.data?.response_text) {
      ...
      return NextResponse.json({ response: text, cached: true, tier: "free", truncated: wasTruncated });
    }
    ...
    if (!apiRes.ok) {
      ...
      return NextResponse.json({ error: data.error?.message ?? `Anthropic returned ${apiRes.status}` }, { status: 502 });
    }
    ...
    if (responseText) {
      await sb.from("ai_cache").insert({ query_hash: queryHash, query_text: query, response_text: responseText });
    }

    const { text, wasTruncated } = truncateToWords(responseText, wordLimit);
    return NextResponse.json({ response: text, cached: false, tier: "free", truncated: wasTruncated });
  } catch (err) {
